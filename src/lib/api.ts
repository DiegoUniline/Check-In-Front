...
class ApiClient {
  ...
  getDashboardTareasCriticas = async () => {
  ...
  getDashboardOcupacionTipo = async () => {
  ...
  // ------- Habitaciones -------
  getHabitaciones = async (params?: Record<string, string>) => {
  ...
  getHabitacion = async (id: string) => {
    const { data, error } = await supabase.from('habitaciones').select('*, tipos_habitacion(*)').eq('id', id).maybeSingle();
    if (error) throw error;
    return data;
  };
  getHabitacionesDisponibles = async (checkin: string, checkout: string, tipoId?: string) => {
  ...
  createHabitacion = async (data: any) => {
  ...
  updateHabitacion = async (id: string, data: any) => {
  ...
  // ------- Reservas -------
  getReservas = async (params?: Record<string, string>) => {
  ...
  cancelarReserva = async (id: string, motivo?: string) => {
  ...
  // ------- Pagos -------
  getPagos = async () => { const { data } = await supabase.from('pagos').select('*').eq('hotel_id', this.hid()).order('fecha', { ascending: false }); return data || []; };
  getPagosReserva = async (reservaId: string) => { const { data } = await supabase.from('pagos').select('*').eq('reserva_id', reservaId).order('fecha', { ascending: false }); return data || []; };
  ...
  deletePago = async (id: string) => { const { error } = await supabase.from('pagos').delete().eq('id', id); if (error) throw error; return { ok: true }; };

  // ------- Cargos -------
  getCargosReserva = async (reservaId: string) => { const { data } = await supabase.from('cargos').select('*').eq('reserva_id', reservaId).order('fecha', { ascending: false }); return data || []; };
  ...
  deleteCargo = async (id: string) => { const { error } = await supabase.from('cargos').delete().eq('id', id); if (error) throw error; return { ok: true }; };
  cargoHabitacion = (data: any) => this.createCargo(data);
  getCargosHabitacion = async (habitacionId: string) => {
    const { data } = await supabase.from('cargos').select('*').eq('habitacion_id', habitacionId);
    return data || [];
  };

  // ------- Conceptos Cargo -------
  getConceptosCargo = async () => { const { data } = await supabase.from('conceptos_cargo').select('*').eq('hotel_id', this.hid()).order('nombre'); return data || []; };
  createConceptoCargo = async (data: any) => { const { data: r, error } = await supabase.from('conceptos_cargo').insert({ ...data, hotel_id: this.hid() }).select().single(); if (error) throw error; return r; };

  // ------- Entregables -------
  getEntregables = async () => { const { data } = await supabase.from('entregables').select('*').eq('hotel_id', this.hid()).order('nombre'); return data || []; };
  createEntregable = async (data: any) => { const { data: r, error } = await supabase.from('entregables').insert({ ...data, hotel_id: this.hid() }).select().single(); if (error) throw error; return r; };
  updateEntregable = async (id: string, data: any) => { const { data: r, error } = await supabase.from('entregables').update(data).eq('id', id).select().single(); if (error) throw error; return r; };
  deleteEntregable = async (id: string) => { const { error } = await supabase.from('entregables').delete().eq('id', id); if (error) throw error; return { ok: true }; };
  getEntregablesReserva = async (reservaId: string) => { const { data } = await supabase.from('entregables_reserva').select('*, entregables(*)').eq('reserva_id', reservaId); return (data || []).map((e: any) => ({ ...e, nombre: e.entregables?.nombre, costo_reposicion: e.entregables?.costo_reposicion })); };
  asignarEntregable = async (reservaId: string, data: any) => { const { data: r, error } = await supabase.from('entregables_reserva').insert({ ...data, reserva_id: reservaId }).select().single(); if (error) throw error; return r; };
  devolverEntregable = async (id: string, data?: any) => { const { data: r, error } = await supabase.from('entregables_reserva').update({ devuelto: true, fecha_devolucion: new Date().toISOString(), ...(data || {}) }).eq('id', id).select().single(); if (error) throw error; return r; };

  // ------- Limpieza -------
  getTareasLimpieza = async (params?: Record<string, string>) => {
  ...
  getTareasLimpiezaHoy = async () => {
    const today = new Date().toISOString().slice(0, 10);
    const { data } = await supabase.from('tareas_limpieza').select('*, habitaciones(numero)').eq('hotel_id', this.hid()).eq('fecha', today);
    return (data || []).map((t: any) => ({ ...t, habitacion_numero: t.habitaciones?.numero }));
  };
  createTareaLimpieza = async (data: any) => { const { data: r, error } = await supabase.from('tareas_limpieza').insert({ ...data, hotel_id: this.hid() }).select().single(); if (error) throw error; return r; };
  updateEstadoLimpieza = async (id: string, estado: string) => { const { data: r, error } = await supabase.from('tareas_limpieza').update({ estado }).eq('id', id).select().single(); if (error) throw error; return r; };
  asignarLimpieza = async (id: string, asignadoA: string, asignadoNombre: string) => { const { data: r, error } = await supabase.from('tareas_limpieza').update({ asignado_a: asignadoA, asignado_nombre: asignadoNombre }).eq('id', id).select().single(); if (error) throw error; return r; };
  deleteTareaLimpieza = async (id: string) => { const { error } = await supabase.from('tareas_limpieza').delete().eq('id', id); if (error) throw error; return { ok: true }; };

  // ------- Mantenimiento -------
  getTareasMantenimiento = async (params?: Record<string, string>) => {
  ...
  getTareasMantenimientoPendientes = async () => {
  ...
  createTareaMantenimiento = async (data: any) => { const { data: r, error } = await supabase.from('tareas_mantenimiento').insert({ ...data, hotel_id: this.hid() }).select().single(); if (error) throw error; return r; };
  updateTareaMantenimiento = async (id: string, data: any) => { const { data: r, error } = await supabase.from('tareas_mantenimiento').update(data).eq('id', id).select().single(); if (error) throw error; return r; };
  updateEstadoMantenimiento = async (id: string, estado: string, costoReal?: number) => { const upd: any = { estado }; if (costoReal !== undefined) upd.costo_real = costoReal; const { data: r, error } = await supabase.from('tareas_mantenimiento').update(upd).eq('id', id).select().single(); if (error) throw error; return r; };
  deleteTareaMantenimiento = async (id: string) => { const { error } = await supabase.from('tareas_mantenimiento').delete().eq('id', id); if (error) throw error; return { ok: true }; };
  ...
  getEmpleado = async (_id: string) => { throw new Error('Use el módulo "Usuarios".'); };
  createEmpleado = async (_data: any) => { throw new Error('Use el módulo "Usuarios".'); };
  updateEmpleado = async (_id: string, _data: any) => { throw new Error('Use el módulo "Usuarios".'); };
  deleteEmpleado = async (_id: string) => { throw new Error('Use el módulo "Usuarios".'); };

  // ------- Productos -------
  getCategorias = async () => { const { data } = await supabase.from('categorias_producto').select('*').eq('hotel_id', this.hid()).order('nombre'); return data || []; };
  createCategoria = async (data: any) => { const { data: r, error } = await supabase.from('categorias_producto').insert({ ...data, hotel_id: this.hid() }).select().single(); if (error) throw error; return r; };
  getProductos = async (params?: Record<string, string>) => {
  ...
  getProducto = async (id: string) => { const { data } = await supabase.from('productos').select('*').eq('id', id).maybeSingle(); return data; };
  createProducto = async (data: any) => { const { data: r, error } = await supabase.from('productos').insert({ ...data, hotel_id: this.hid() }).select().single(); if (error) throw error; return r; };
  updateProducto = async (id: string, data: any) => { const { data: r, error } = await supabase.from('productos').update(data).eq('id', id).select().single(); if (error) throw error; return r; };
  deleteProducto = async (id: string) => { const { error } = await supabase.from('productos').delete().eq('id', id); if (error) throw error; return { ok: true }; };
  movimientoInventario = async (id: string, data: any) => {
  ...
  getMovimientosProducto = async (id: string) => { const { data } = await supabase.from('movimientos_inventario').select('*').eq('producto_id', id).order('created_at', { ascending: false }); return data || []; };

  // ------- Gastos -------
  getGastos = async (params?: Record<string, string>) => {
    let q = supabase.from('gastos').select('*').eq('hotel_id', this.hid()).order('fecha', { ascending: false });
    if (params?.categoria) q = q.eq('categoria', params.categoria);
    const { data } = await q;
    return data || [];
  };
  getGasto = async (id: string) => { const { data } = await supabase.from('gastos').select('*').eq('id', id).maybeSingle(); return data; };
  getCategoriasGastos = async () => ['Servicios', 'Suministros', 'Mantenimiento', 'Salarios', 'Marketing', 'Otros'];
  getResumenGastos = async (fechaDesde: string, fechaHasta: string) => {
  ...
  createGasto = async (data: any) => { const { data: r, error } = await supabase.from('gastos').insert({ ...data, hotel_id: this.hid() }).select().single(); if (error) throw error; return r; };
  updateGasto = async (id: string, data: any) => { const { data: r, error } = await supabase.from('gastos').update(data).eq('id', id).select().single(); if (error) throw error; return r; };
  deleteGasto = async (id: string) => { const { error } = await supabase.from('gastos').delete().eq('id', id); if (error) throw error; return { ok: true }; };

  // ------- Proveedores -------
  getProveedores = async () => { const { data } = await supabase.from('proveedores').select('*').eq('hotel_id', this.hid()).order('nombre'); return data || []; };
  getProveedor = async (id: string) => { const { data } = await supabase.from('proveedores').select('*').eq('id', id).maybeSingle(); return data; };
  createProveedor = async (data: any) => { const { data: r, error } = await supabase.from('proveedores').insert({ ...data, hotel_id: this.hid() }).select().single(); if (error) throw error; return r; };
  updateProveedor = async (id: string, data: any) => { const { data: r, error } = await supabase.from('proveedores').update(data).eq('id', id).select().single(); if (error) throw error; return r; };
  deleteProveedor = async (id: string) => { const { error } = await supabase.from('proveedores').delete().eq('id', id); if (error) throw error; return { ok: true }; };

  // ------- Compras -------
  getCompras = async () => { const { data } = await supabase.from('compras').select('*').eq('hotel_id', this.hid()).order('fecha', { ascending: false }); return data || []; };
  getCompra = async (id: string) => { const { data } = await supabase.from('compras').select('*, compras_detalle(*)').eq('id', id).maybeSingle(); return data; };
  createCompra = async (data: any) => { const { detalles, ...header } = data; const { data: r, error } = await supabase.from('compras').insert({ ...header, hotel_id: this.hid() }).select().single(); if (error) throw error; if (Array.isArray(detalles) && detalles.length) { await supabase.from('compras_detalle').insert(detalles.map((d: any) => ({ ...d, compra_id: r.id }))); } return r; };
  updateCompra = async (id: string, data: any) => { const { data: r, error } = await supabase.from('compras').update(data).eq('id', id).select().single(); if (error) throw error; return r; };
  updateEstadoCompra = async (id: string, estado: string) => { const { data: r, error } = await supabase.from('compras').update({ estado }).eq('id', id).select().single(); if (error) throw error; return r; };
  deleteCompra = async (id: string) => { const { error } = await supabase.from('compras').delete().eq('id', id); if (error) throw error; return { ok: true }; };

  // ------- Ventas -------
  getVentas = async () => { const { data } = await supabase.from('ventas').select('*').eq('hotel_id', this.hid()).order('fecha', { ascending: false }); return data || []; };
  getVenta = async (id: string) => { const { data } = await supabase.from('ventas').select('*, ventas_detalle(*)').eq('id', id).maybeSingle(); return data; };
  ...
  // ------- Transacciones -------
  getTransacciones = async () => { const { data } = await supabase.from('transacciones').select('*').eq('hotel_id', this.hid()).order('fecha', { ascending: false }); return data || []; };
  getTransaccion = async (id: string) => { const { data } = await supabase.from('transacciones').select('*').eq('id', id).maybeSingle(); return data; };

  // ------- Hotel -------
  getHotel = async () => { const { data } = await supabase.from('hotels').select('*').eq('id', this.hid()).maybeSingle(); return data; };
  updateHotel = async (data: any) => { const { data: r, error } = await supabase.from('hotels').update(data).eq('id', this.hid()).select().single(); if (error) throw error; return r; };

  // ------- SAAS (stubs vacíos) -------
  getCuentas = async () => [];
  createCuenta = async (_data: any) => ({});
  updateCuenta = async (_id: string, _data: any) => ({});
  deleteCuenta = async (_id: string) => ({});
  getPlanes = async () => [];
  createPlan = async (_data: any) => ({});
  updatePlan = async (_id: string, _data: any) => ({});
  getSuscripcionesGlobales = async () => [];
  createSuscripcion = async (_data: any) => ({});
  extenderSuscripcion = async (_id: string, _dias = 30) => ({});
  eliminarSuscripcion = async (_id: string) => ({});
  getHotelesSaas = async () => { const { data } = await supabase.from('hotels').select('*'); return data || []; };
  createHotelSaas = async (data: any) => { const { data: r, error } = await supabase.from('hotels').insert(data).select().single(); if (error) throw error; return r; };
  registrarHotelFull = async (_data: any) => ({});
  asignarHotelACuenta = async (_data: any) => ({});
  getMiSuscripcion = async () => ({ activa: true, plan: 'Demo', vence: '2099-12-31' });

  // ------- Usuarios -------
  getUsuarios = async () => { const { data } = await supabase.from('profiles').select('*').eq('hotel_id', this.hid()); return (data || []).map((u: any) => ({ ...u, rol: 'Recepcion' })); };
  getUsuario = async (id: string) => { const { data } = await supabase.from('profiles').select('*').eq('id', id).maybeSingle(); return data; };
  createUsuario = async (_data: any) => { throw new Error('La creación de usuarios se hace desde Configuración / Auth.'); };
  updateUsuario = async (id: string, data: any) => { const { data: r, error } = await supabase.from('profiles').update(data).eq('id', id).select().single(); if (error) throw error; return r; };
  deleteUsuario = async (_id: string) => { throw new Error('Eliminación de usuarios deshabilitada en demo.'); };
  getRoles = async () => ['Admin', 'Recepcion', 'Housekeeping', 'Mantenimiento', 'Gerente'];
