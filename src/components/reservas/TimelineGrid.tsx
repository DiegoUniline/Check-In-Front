...
export function TimelineGrid({
  ...
  return (
    <div 
      className="border rounded-lg overflow-hidden bg-card flex flex-col w-full h-[calc(100vh-280px)]"
    ...
    >
      <ScrollArea className="flex-1">
        <div className="min-w-max" ref={gridRef}>
          <div className="flex border-b bg-muted/30 sticky top-0 z-20">
            <div 
              className="flex-shrink-0 px-3 py-2 font-medium text-sm border-r bg-muted/50 sticky left-0 z-30"
          ...
          {habitaciones.map((habitacion) => (
            <div key={habitacion.id} className="flex border-b hover:bg-muted/10 group">
              <div 
                className="flex-shrink-0 px-3 py-2 border-r bg-muted/20 flex items-center gap-2 sticky left-0 z-10 group-hover:bg-muted/30 transition-colors"
                style={{ width: roomColumnWidth }}
              >
                <div className="flex flex-col">
                  <span className="font-medium text-sm">{habitacion.numero}</span>
                  <span className="text-xs text-muted-foreground truncate w-20">
                    {habitacion.tipo_codigo || habitacion.tipo_nombre}
                  </span>
                </div>
                {habitacion.estado_limpieza === 'Sucia' && <Badge variant="outline" className="text-[10px] px-1 py-0">🧹</Badge>}
                      ...
                      <TooltipTrigger asChild>
                        <div
                          className={cn(
                            ...
                            !reserva && !isSelected && "hover:bg-primary/5 cursor-crosshair",
                        ...
                        >
                          {reserva && position && (
                            ...
                            >
                              ...
                              {position === 'middle' && <span className="opacity-60 w-full text-center">─</span>}
                              {position === 'end' && <span className="opacity-70 text-[9px] w-full text-center">salida</span>}
                            </div>
                          )}
                        </div>
                      </TooltipTrigger>
      ...
      <div className="p-2 bg-muted/30 border-t flex items-center justify-between text-xs text-muted-foreground flex-shrink-0">
        ...
        <span className="hidden md:inline">💡 Arrastra sobre las celdas vacías para crear una nueva reserva</span>
      </div>
    </div>
  );
}
